...
class Pregunta:
    ...
    def composa_titol(self, num, ambpes):
        """ composa el títol de la pregunta en forma de llista de
        línies a mostrar.
        ...
        return [ "", titol, subratllat(titol), "" ]

    def composa_respostes(self, ambpes):
        """ composa les respostes de la pregunta en forma de llista de 
        línies a mostrar.
        En cas que ambpes sigui cert, mostra també els pesos amb
        les respostes. """
        lrespostes = []
        if len(self.respostes) > 0:
            lrespostes += [ "-"*4, "" ] # separació de les respostes 

            # composa les diferents respostes
            respostes = self.respostes + self.finals
            for i in range(len(respostes)):
                if ambpes:
                    pes = "[%.2f] "%respostes[i][1]
                else:
                    pes = ""
                resp = "%s*%s)* %s"%(pes, chr(ord("a")+i), respostes[i][0])
                lrespostes += [ resp, "" ]
        return lrespostes
    ...
    def mostra_pregunta(self, num=0, ambpes=False):
        """ mostra la pregunta amb el número indicat.
            En cas que ambpes sigui cert, mostra també els pesos amb
            les respostes. """
        lpregunta = self.composa_titol(num, ambpes)
        lpregunta += [ self.enunciat, "" ]
        lpregunta += self.composa_respostes(ambpes)
        for l in lpregunta: print l
...
def split_resposta(linia):
    ...
    return res
#
def es_comentari(lin):
    """ true if lin is a comment """
    return lin.startswith(".. #") or lin.startswith(".. /")

def es_inici_pregunta(lin):
    """ true if lin is the start of a question """
    return lin.startswith(".. pregunta:")
#
def es_inici_enunciat(lin):
    """ true if lin is the start of a question """
    return lin.startswith(".. enunciat:")
#
def es_inici_resposta(lin):
    """ true if lin is the start of a answer """
    return lin.startswith(".. resposta:")
#
def processa_continguts(f):
    ...
    for lin in f:
        nlin += 1
        if es_comentari(lin):   # és un comentari
            pass
        elif estat == "pregunta":
            if es_inici_pregunta(lin):
                estat = "títol"  # ha trobat una pregunta i mira d'obtenir el títol
            else:
                # ignora
                pass
        elif estat == "títol":
            if es_inici_enunciat(lin):   # s'ha completat el títol
                estat = "enunciat"      # s'ha llegit el títol, ara toca l'enunciat
            else:   # forma part del títol
                titol += lin
        elif estat == "enunciat":
            if es_inici_resposta(lin):      # s'ha finalitzat l'enunciat 
                pes, final = split_resposta(lin)
                estat = "resposta"          # s'està llegint una resposta
            elif es_inici_pregunta(lin):    # pregunta anterior sense respostes
            ...
            if es_inici_pregunta(lin):  # s'han acabat les respostes
...
#
# helping functions
def subratllat(sentence, char = "-"):
    """ returns a string with sentence length composed by char
    characters """
    return char * len(sentence.decode("utf-8"))
